...
**1）静态申请**：
驱动开发者选择一个数字作为主设备号，然后通过宏函数 ```MKDEV(major,minor)``` 来生成一个设备号，再由函数 ```int register_chrdev_region(dev_t, unsigned, const char *)``` 向内核注册。缺点：如果注册申请的设备号已被内核使用，则申请失败。
**2）动态分配**：
开发者使用函数 ```int alloc_chrdev_region(dev_t *, unsigned, unsigned, const char *)``` 由内核注册，由内核配分一个可用的主设备号。优点：由于内核已经知道那些设备号被使用，所以不会分配到已被使用的设备号上。
**3）设备号注销**
当我们卸载设备时，需要将设备号归还给内核，这是需要用到设备号注销函数。在使用上述 2 种设备号申请方式后，可使用函数 ```void unregister_chrdev_region(dev_t from, unsigned count)``` 进行设备号注销。
